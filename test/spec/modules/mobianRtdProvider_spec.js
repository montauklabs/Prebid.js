...
import {
  ...
  VIEWABILITY_TARGETING_MAX_CACHE_SIZE,
  VIEWABILITY_KEYS,
  VP,
...
describe('Mobian RTD Submodule', function () {
  ...
  describe('fetchViewabilityData', function () {
    it('should request viewability using the full page URL, placement source, and ad unit code', async function () {
      ...
      try {
        history.pushState({}, '', '/viewability-page?ignored=true#ignored');
  ...
  describe('makeMemoizedViewabilityFetch', function () {
    it('should use the dedicated viewability cache size', async function () {
      let fetchCount = 0;
      ajaxStub = sinon.stub(dep, 'ajaxBuilder').returns(function (url, callbacks) {
        fetchCount++;
        callbacks.success(mockViewabilityResponse);
      ...
      try {
        for (let i = 0; i < VIEWABILITY_TARGETING_MAX_CACHE_SIZE; i++) {
          history.pushState({}, '', `/viewability-cache-size-${i}`);
          await memoizedFetch(getPageUrl(), 'ad-unit-1', 'gam_ad_unit');
        }
        expect(fetchCount).to.equal(VIEWABILITY_TARGETING_MAX_CACHE_SIZE);

        history.pushState({}, '', '/viewability-cache-size-overflow');
        await memoizedFetch(getPageUrl(), 'ad-unit-1', 'gam_ad_unit');
        expect(fetchCount).to.equal(VIEWABILITY_TARGETING_MAX_CACHE_SIZE + 1);

        history.pushState({}, '', '/viewability-cache-size-0');
        await memoizedFetch(getPageUrl(), 'ad-unit-1', 'gam_ad_unit');
        expect(fetchCount).to.equal(VIEWABILITY_TARGETING_MAX_CACHE_SIZE + 2);
      } finally {
        history.replaceState({}, '', originalHref);
      }
    });
